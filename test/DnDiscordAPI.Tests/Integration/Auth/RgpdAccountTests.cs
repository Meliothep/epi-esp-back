...
public sealed class RgpdAccountTests
{
    ...
    public async Task DeleteMe_RemovesCharacters_Campaigns_Memberships_AndBlocksFurtherCalls()
    {
        ...
        // User B rejoint la campagne de A (via public join pour simplicité).
        // Si le join nécessite un invite code ou autre, le test peut être
        // adapté ; ici on crée directement le membership via l'API.
        ...
        // Assert 3 — les personnages de A sont purgés.
        var charactersClientA = _fixture.CreateAuthenticatedClient($"rgpd-a-check-{Guid.NewGuid():N}");
        var listA = await clientA.GetAsync("/api/games/character/my-characters");
        // clientA reçoit 401 (compte supprimé), donc on ne peut plus lister
        // avec son token. On vérifie via un autre client qui ne touche PAS
        // les Characters de A — la purge sera confirmée implicitement par
        // le fait que recréer un compte avec le même DiscordId est bloqué.
        Assert.Equal(HttpStatusCode.Unauthorized, listA.StatusCode);

        // Assert 4 — la campagne possédée par A n'existe plus (404 pour B).
    ...
    private static async Task JoinCampaignAsMember(HttpClient client, Guid campaignId)
    {
        ...
        // Ne pas assert : l'API de join peut varier ou requérir un code.
        _ = response;
    }
